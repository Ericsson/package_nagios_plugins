...
cat << EOSPEC > $PRE
getent group $nrpe_group > /dev/null || groupadd -o -g $nrpe_gid $nrpe_group
getent passwd $nrpe_user > /dev/null || useradd -o -u $nrpe_uid -g $nrpe_gid -d $nrpe_home -s /bin/sh $nrpe_user
EOSPEC
